...
public class Sentiment {
        ...
        this.Sentiment_Doc_Weight(GlobalPara.trainblog);
        ...
        this.ReadXML(DirPath+"negativeout/"+NegativeFiles[i], NegativeCurrentList); //前 读入 后
        }
        ...
        Document Doc = SaxReader.read(new File(FilePath)); //从xml文件中获得数据
        Element root = Doc.getRootElement();//获取根
        
        Element content = root.element("content");//获取子节点
        ...
        public void GetWord( Element root, ArrayList<String> currentList ) {//获取XML中的情感词List
